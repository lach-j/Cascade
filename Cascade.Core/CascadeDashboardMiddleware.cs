...
public class CascadeDashboardMiddleware
{
    private readonly RequestDelegate _next;
    private readonly ILogger<CascadeDashboardMiddleware> _logger;
    ...
    public CascadeDashboardMiddleware(
        RequestDelegate next,
        ILogger<CascadeDashboardMiddleware> logger,
    ...
    private async Task ServeEmbeddedResource(HttpContext context)
    {
        ...
        var assembly = typeof(CascadeDashboardMiddleware).Assembly;
        var requestPath = path == "/" ? "/index.html" : path;
        var resourcePath = $"Cascade.Core.Dashboard{requestPath.Replace('/', '.')}";
        var extension = Path.GetExtension(requestPath);

        using var stream = assembly.GetManifestResourceStream(resourcePath);
        if (stream == null)
        {
            var fallbackStream = assembly.GetManifestResourceStream("Cascade.Core.Dashboard.index.html");
